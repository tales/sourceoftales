<tileset name="plowed_soil" tilewidth="32" tileheight="32">
 <image source="./plowed_soil.png" width="96" height="160"/>
 <terraintypes>
  <terrain name="PlowedSoil" tile="10"/>
  <terrain name="Grass" tile="12"/>
 </terraintypes>
 <tile id="0" terrain="1,0,0,1"/>
 <tile id="1" terrain="0,0,0,1"/>
 <tile id="2" terrain="0,0,1,0"/>
 <tile id="3" terrain="0,1,1,0"/>
 <tile id="4" terrain="0,1,0,0"/>
 <tile id="5" terrain="1,0,0,0"/>
 <tile id="6" terrain="1,1,1,0"/>
 <tile id="7" terrain="1,1,0,0"/>
 <tile id="8" terrain="1,1,0,1"/>
 <tile id="9" terrain="1,0,1,0"/>
 <tile id="10" terrain="0,0,0,0"/>
 <tile id="11" terrain="0,1,0,1"/>
 <tile id="12" terrain="1,0,1,1"/>
 <tile id="13" terrain="0,0,1,1"/>
 <tile id="14" terrain="0,1,1,1"/>
</tileset>
